...
class RpmsRpc::MappingsTest < Minitest::Test
  ...
  # Its sentinel "^No vitals found." (ORQQVI.m:24) never parses as data —
  # main's sentinel guard (#218) drops the row outright, which is stronger
  # than the "parses with an empty IEN" this test originally asserted.
  def test_vitals_for_date_range_sentinel_is_not_data
  ...
  # Its sentinel "^No medications found." (ORQQPS.m:53) never parses as data
  # — the sentinel guard (#218) drops the row outright.
  def test_medication_list_sentinel_is_not_data
